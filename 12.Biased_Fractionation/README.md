...
```shell
# Step 1: Generate REF-biased VCF file
# Purpose: Reduce reference bias using outgroup information
python reduce_ref_bias_on_outgroup.py \
    -i input.vcf \
    -g outgroup.txt \
    -o input.REF.biased.vcf

# File Specification: "outgroup.txt"
# Format: Tab-delimited, no header
# Columns:
#   1. Outgroup sample name
#   2. Taxonomic group name
# Example:
#   Outgroup1\tWild_ancestor
#   Outgroup2\tDomesticated_variant

# Step 2: Extract region-specific VCFs
# Purpose: Create subset VCFs based on LF/HF BED regions
# (Implementation depends on specific BED tools, e.g. bcftools)

# Step 3: EasySFS Analysis
# Analysis for LF regions with 0-fold synonymous SNPs
# Using 108 cultivated haplotypes, alternative, use --proj 9 for 10 bins
python /home/wangnan/Software/easySFS/easySFS.py \
    -i 0-fold-LF.vcf \
    --unfolded \
    -p cultivar.pop \
    -a \
