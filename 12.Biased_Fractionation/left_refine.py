...
def find_nearest(query_file, chrom_index):
    ...
    with open(query_file, 'r') as f:
        for line in f:
            ...
            # 二分查找最近位置
            idx = bisect.bisect_left(positions, q_pos)
            candidates = []
            
            if idx > 0:
                candidates.append((positions[idx-1], genes[idx-1]))
            if idx < len(positions):
                candidates.append((positions[idx], genes[idx]))
            ...
            # 找出绝对距离最小的
            nearest = min(candidates, key=lambda x: abs(x[0] - q_pos))
            distance = abs(nearest[0] - q_pos)
            results.append(f"{query_gene}\t{nearest[1]}\t{q_chrom}\t{distance}")
...
if __name__ == "__main__":
    ...
    # 输出结果
    print("QueryGene\tNearestgleft\tChromosome\tDistance")
    for line in output:
        print(line)
